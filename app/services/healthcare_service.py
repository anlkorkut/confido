...
class HealthcareService:
    ...
    async def check_appointment_availability(self, date: str, time: str, doctor: Optional[str] = None) -> Dict:
        ...
        # Define standard available time slots
        standard_slots = ["9:00", "10:00", "13:00", "14:00", "16:00"]
        
        # Mark slot as available if it matches one of the standard slots
        is_available = time in standard_slots
        
        # Ensure requested time is in available slots if it's a standard slot
        if is_available and doctor:
            has_requested_time = any(slot["time"] == time and slot["doctor"] == doctor for slot in available_slots)
            if not has_requested_time:
                available_slots.append({
                    "doctor": doctor,
                    "date": target_date.strftime("%Y-%m-%d"),
                    "time": time,
        ...
        return {
            ...
            "is_available": is_available,  # Add this key
    ...
    async def get_clinic_info(self, query_type: str) -> Dict:
        ...
        clinic_info = {
            ...
            "doctors": [
                ...
                {"name": "Dr. Joy Jackson", "specialty": "Eye"},
